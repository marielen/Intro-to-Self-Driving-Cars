...
vector < vector <float> > blur(vector < vector < float> >& grid, float blurring) {
	...
	vector < vector <float> > window (3, vector<float> (3)), new_matrix;
	...
	for (int i = 0; i < height; i++){
		new_matrix.push_back(vector <float>(width, 0));
	}
		...
		for (int j = 0; j < width; j++){
			...
			for (int dx = -1; dx < 2; dx++){
				for (int dy = -1; dy < 2; dy++){
					...
					new_matrix[new_i][new_j] = new_matrix[new_i][new_j] * grid_val;
				}
			}
		}
	}
